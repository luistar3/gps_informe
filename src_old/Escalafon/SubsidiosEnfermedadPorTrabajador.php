...
try{
    if(empty($Api_id_get_first) || $Api_id_get_first == -1){
        $arrayReturn['mensaje'] = 'Ingrese un id trabajador';
		$arrayReturn['error']   = false;
		http_response_code(200);
    }
    else{
        $dtListarRegistros = $reportesController->fncReporteSubsidiosPorEnfermedad($Api_id_get_first);
