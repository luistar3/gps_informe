...
require_once("../../App/Escalafon/Controllers/EvaluacionController.php");
...
try
{
	...
	{
		$evaluaciones = $evaluacionController->fncListar($Api_id_get_first);

		if(fncGeneralValidarDataArray($evaluaciones))
		{
		...
		$arrayReturn["data"]	= $evaluaciones;

	}
	else if ($Api_id_get_first != -1 && $Api_id_get_second != -1)
	{
		$evaluacion = $evaluacionController->fncObtener($Api_id_get_first, $Api_id_get_second);
