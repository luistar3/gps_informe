...
if (
	(
		!isset(
			$input->id_proceso_administrativo_disciplinario,
			$input->id_tipo_recurso_impugnativo
		)
	) 
)
{
...
try
{
	if (!isset($input->id_recurso_impugnativo))
	{
		$recurso = $recursoController->fncGuardar($input, $files);
	...
	else if (isset($input->id_recurso_impugnativo) && !empty($input->id_recurso_impugnativo))
	{
		$recurso = $recursoController->fncActualizar($input, $files);
