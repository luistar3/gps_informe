...
try
{
	...
	{
		$procesos = $procesoController->fncListar($Api_id_get_first);

		if(fncGeneralValidarDataArray($procesos))
		{
		...
		$arrayReturn["data"]	= $procesos;

	}
	else if ($Api_id_get_first != -1 && $Api_id_get_second != -1)
	{
		$proceso = $procesoController->fncObtener($Api_id_get_first, $Api_id_get_second);
