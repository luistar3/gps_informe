...
try
{
	...
	{
		$subsidios = $subsidioController->fncListar($Api_id_get_first);

		if(fncGeneralValidarDataArray($subsidios))
		{
		...
		$arrayReturn["data"]	= $subsidios;

	}
	else if ($Api_id_get_first != -1 && $Api_id_get_second != -1)
	{
		$subsidio = $subsidioController->fncObtener($Api_id_get_first, $Api_id_get_second);
