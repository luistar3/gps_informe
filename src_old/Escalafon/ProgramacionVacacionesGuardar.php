...
$clsProgramacionVacionesController = new ProgramacionVacacionesController();
...
if(fncGeneralValidarDataArray($arrayParametrosInput)){
	...
	if($archivoPermitido=="si"){
		if(fncValidarMalicioso($parametroArchivo["name"])==0){
			try{
				$dtGuardar = $clsProgramacionVacionesController->fncGuardar($arrayParametrosInput,$parametroArchivo);
