...
try
{
	...
	{
		$constancias = $constanciaController->fncListar($Api_id_get_first);

		if(fncGeneralValidarDataArray($constancias))
		{
		...
		$arrayReturn["data"]	= $constancias;

	}
	else if ($Api_id_get_first != -1 && $Api_id_get_second != -1)
	{
		$constancia = $constanciaController->fncObtener($Api_id_get_first, $Api_id_get_second);
