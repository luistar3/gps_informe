<?php
require_once('../../App/Controllers/JuridicoController.php');
...
// Controllers
$ClsJuridicoController = new JuridicoController;
...
try {
	$rucExitente = $ClsJuridicoController->fncObtenerRegistroRuc($input);
	if (!empty($rucExitente) && !isset($input->idJuridico)) {
		http_response_code(422);
		$arrayReturn["mensaje"] = "Ruc ya registrado en el Sistema";
		$arrayReturn["error"]	= true;
		echo json_encode($arrayReturn, JSON_UNESCAPED_UNICODE);
		exit;
	}
	if (!isset($input->idJuridico)) {
		$personaJuridica = $ClsJuridicoController->fncGuardar2($input);

		if ($personaJuridica) {
			http_response_code(201);
			$arrayReturn["mensaje"]	= "Datos guardados con éxito";
			$arrayReturn["error"]	= false;
			$arrayReturn["data"]	= $personaJuridica;
		} else {
			http_response_code(500);
			$arrayReturn["mensaje"]	= "Error al guardar los datos";
			$arrayReturn["error"]	= true;
		}
	} else if (isset($input->idJuridico)) {
		$personaJuridica = $ClsJuridicoController->fncActualizar($input);
